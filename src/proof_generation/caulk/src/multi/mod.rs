...
pub mod setup;
mod unity;
#[path = "../../../ZIP_proof_generation/ZIP_lookup/config.rs"]
mod config;
use config::{LookupConfig, parse_hex_file_to_fr};
...
pub fn get_poly_and_g2_openings<E: PairingEngine>(
    ...
    match res {
        ...
        Err(_) => {
            // Load table coefficients from the config.rs
            let cfg = LookupConfig::load();
            let table_path = cfg.table_dir.join(&cfg.target_filename);
            //println!("Loading table from: {}", table_path.display());
            let mut coeffs = parse_hex_file_to_fr::<E>(&table_path);
 
            if cfg.pad_to_pow2 {
                let orig = coeffs.len();
                let target_len = orig.next_power_of_two();
                if target_len > pp.N {
                    panic!(
                        "Padded length {} exceeds SRS domain size N={}. \
                         Re-run with larger n (so N >= {}), or disable padding.",
                        target_len, pp.N, target_len
                    );
                }
                if target_len > orig {
                    coeffs.resize(target_len, E::Fr::one());
                    //println!("Padded coefficients from {} -> {} with 1s", orig, target_len);
                }
            }
       
            let c_poly = DensePolynomial::from_coefficients_vec(coeffs);

            /*
            for (i, coeff) in c_poly.coeffs.iter().enumerate() {
