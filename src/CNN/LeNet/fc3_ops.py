...
X = np.array(acts["fc3_in"], dtype=np.float64)  # [1,84]

with open("lenet_model_params.json") as f: params = json.load(f)
W = np.array(params["fc3.weight"], dtype=np.float64)  # [10,84]
b = np.array(params["fc3.bias"],   dtype=np.float64)  # [10]
...
print(f"[fc3] multiplications   logged: {mul_count}")
print(f"[fc3] additions         logged: {add_count}")
