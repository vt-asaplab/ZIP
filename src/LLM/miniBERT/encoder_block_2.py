...
# 1) Get post-LN input to Block 2
inp_pre = np.array(acts['bert.encoder.layer.1.output.LayerNorm'], dtype=float)  # [B, S, H]
ln1_w_prev = np.array(params['bert.encoder.layer.1.output.LayerNorm.weight'], dtype=float)
ln1_b_prev = np.array(params['bert.encoder.layer.1.output.LayerNorm.bias'],   dtype=float)

mu1_prev  = inp_pre.mean(axis=-1, keepdims=True)
var1_prev = inp_pre.var(axis=-1, keepdims=True)
x = (inp_pre - mu1_prev) / np.sqrt(var1_prev + 1e-12) * ln1_w_prev + ln1_b_prev  # [B,S,H]

# 2) Q/K/V projections (matmul + bias add + logging)
Wq = np.array(params['bert.encoder.layer.2.attention.self.query.weight'], dtype=float)
Bq = np.array(params['bert.encoder.layer.2.attention.self.query.bias'],   dtype=float)
Wk = np.array(params['bert.encoder.layer.2.attention.self.key.weight'],   dtype=float)
Bk = np.array(params['bert.encoder.layer.2.attention.self.key.bias'],     dtype=float)
Wv = np.array(params['bert.encoder.layer.2.attention.self.value.weight'], dtype=float)
Bv = np.array(params['bert.encoder.layer.2.attention.self.value.bias'],   dtype=float)

Bsz, seq_len, hidden = x.shape

mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_0.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_0.txt")
...
# 4.1) qh @ kh^T
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_1.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_1.txt")
...
with open(os.path.join(DIR_NONLIN, "inv_sqrt_0_block2.txt"), "w") as nf:
...
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_2.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_2.txt")
...
mask_add_path = os.path.join(DIR_ADD, "encoder_block_2_add_3.txt")
mask_mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_3.txt")

mul_count3 = add_count3 = 0
with open(mask_mul_path, "w", newline="") as mf, open(mask_add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","head","i","j","value1","value2","product"])
    aw.writerow(["batch","head","i","j","prev_val","mask_val","new_val"])
    for b in range(B):
        for h in range(Nh):
            for i in range(S):
                for j in range(S):
...
exp_path = os.path.join(DIR_NONLIN, "exp_2.txt")
...
# Sum along last axis
sum_add_path = os.path.join(DIR_ADD, "encoder_block_2_add_4.txt")
sum_mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_4.txt")
...
# Reciprocal
div_path = os.path.join(DIR_DIV, "encoder_block_2_div_0.txt")
...
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_5.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_5.txt")
...
# Context: ctx = attn @ vh
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_6.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_6.txt")
...
Wout = np.array(params['bert.encoder.layer.2.attention.output.dense.weight'], dtype=float)
Bout = np.array(params['bert.encoder.layer.2.attention.output.dense.bias'],   dtype=float)

mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_7.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_7.txt")
...
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_8.txt")
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_8.txt")
...
with open(add_path, "w", newline="") as af, open(mul_path, "w", newline="") as mf:
    ...
    aw.writerow(["batch","seq","feat","attn_out_val","x_val","sum"])
...
# LN1 params (block2)
ln1_w = np.array(params['bert.encoder.layer.2.attention.output.LayerNorm.weight'], dtype=float)
ln1_b = np.array(params['bert.encoder.layer.2.attention.output.LayerNorm.bias'],   dtype=float)

# m1 = mean(res1)
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_9.txt")
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_9.txt")
...
# y1 = res1 - m1  -> log & count as addition with negated second operand
add_path_sub0 = os.path.join(DIR_ADD, "encoder_block_2_add_9a.txt")
...
# y2 = y1 * y1
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_10.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_10.txt")
y2 = np.zeros_like(y1, dtype=np.float64)

mul_count10 = add_count10 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","seq","feat","y1_val","y1_val_again","product"])
...
# y3 = sum(y2)
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_11.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_11.txt")
...
# v1 = y3 * inv_H
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_12.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_12.txt")
v1 = np.zeros_like(y3, dtype=np.float64)

mul_count12 = add_count12 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","seq","feat","y3_val","inv_H","product"])
...
# y4 = v1 + eps
eps = 1e-12
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_13.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_13.txt")
y4 = np.zeros_like(v1, dtype=np.float64)
mul_count13 = add_count13 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["(no multiplications in y4 = v1 + eps)"])
    aw.writerow(["batch","seq","feat","prev_val","eps","new_val"])
    for b in range(B):
        for s_idx in range(S):
            prev = v1[b, s_idx, 0]
...
# Save y4 as inv_sqrt_1_block2.txt
inv1_path = os.path.join(DIR_NONLIN, "inv_sqrt_1_block2.txt")
bound_lo, bound_hi = 0.001, 210.0
...
# y5 = y1 * inv_sqrt2
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_14.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_14.txt")
y5 = np.zeros_like(y1, dtype=np.float64)

mul_count14 = add_count14 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","seq","feat","y1_val","inv_sqrt2_val","product"])
...
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_15.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_15.txt")
ln1_out = np.zeros_like(y5, dtype=np.float64)

mul_count15 = add_count15 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","seq","feat","y5_val","ln1_w_val","product"])
    aw.writerow(["batch","seq","feat","prev_val","bias_val","new_val"])
    for b in range(B):
        for s_idx in range(S):
            for f in range(H):
                yv   = y5[b, s_idx, f]
...
# 7) Feed-Forward Network (dense in/out + GELU)
Wi = np.array(params['bert.encoder.layer.2.intermediate.dense.weight'], dtype=float)
Bi = np.array(params['bert.encoder.layer.2.intermediate.dense.bias'],   dtype=float)
Wo = np.array(params['bert.encoder.layer.2.output.dense.weight'],       dtype=float)
Bo = np.array(params['bert.encoder.layer.2.output.dense.bias'],         dtype=float)

# hidden = ln1_out @ Wi.T + Bi
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_16.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_16.txt")

Bsz, S, H_in = ln1_out.shape
H_out, H_chk = Wi.shape
...
# GELU inputs
gelu_in_path = os.path.join(DIR_NONLIN, "gelu_2.txt")
count_gelu_in = 0
bound = 5.0
...
# ffn = act @ Wo.T + Bo
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_17.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_17.txt")

Bsz, S, H_in2  = act.shape
H_out2, H_chk2 = Wo.shape
...
# res2 = ffn + ln1_out
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_18.txt")
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_18.txt")
res2 = np.zeros_like(ffn, dtype=np.float64)

add_count18 = mul_count18 = 0
with open(add_path, "w", newline="") as af, open(mul_path, "w", newline="") as mf:
    aw = csv.writer(af); mw = csv.writer(mf)
    aw.writerow(["batch","seq","feat","ffn_val","ln1_out_val","sum"])
...
# LN2 params (block2)
ln2_w = np.array(params['bert.encoder.layer.2.output.LayerNorm.weight'], dtype=float)
ln2_b = np.array(params['bert.encoder.layer.2.output.LayerNorm.bias'],   dtype=float)

# m2
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_19.txt")
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_19.txt")
...
add_path_sub1 = os.path.join(DIR_ADD, "encoder_block_2_add_19a.txt")
...
# t2 = t1 * t1
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_20.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_20.txt")
t2 = np.zeros_like(t1, dtype=np.float64)
mul_count20 = add_count20 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","seq","feat","t1_val","t1_val_again","product"])
...
# t3 = sum(t2)
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_21.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_21.txt")
...
# v2 = t3 * inv_H
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_22.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_22.txt")
v2 = np.zeros_like(t3, dtype=np.float64)
mul_count22 = add_count22 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","seq","feat","t3_val","inv_H","product"])
...
# t4 = v2 + eps
eps = 1e-12
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_23.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_23.txt")
...
# Save t4 as inv_sqrt_2_block2.txt
t4_path = os.path.join(DIR_NONLIN, "inv_sqrt_2_block2.txt")
bound_lo, bound_hi = 0.001, 210.0
...
# t5 = t1 * inv_sqrt3
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_24.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_24.txt")
t5 = np.zeros_like(t1, dtype=np.float64)
mul_count24 = add_count24 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","seq","feat","t1_val","inv_sqrt3_val","product"])
...
mul_path = os.path.join(DIR_MUL, "encoder_block_2_mul_25.txt")
add_path = os.path.join(DIR_ADD, "encoder_block_2_add_25.txt")
ln2_out = np.zeros_like(t5, dtype=np.float64)
mul_count25 = add_count25 = 0
with open(mul_path, "w", newline="") as mf, open(add_path, "w", newline="") as af:
    mw = csv.writer(mf); aw = csv.writer(af)
    mw.writerow(["batch","seq","feat","t5_val","ln2_w_val","product"])
    aw.writerow(["batch","seq","feat","prev_val","bias_val","new_val"])
    for b in range(B):
        for s_idx in range(S):
            for f in range(H):
...
# Final output
print("Encoder Block 2 output (post-LN) shape:", ln2_out.shape)
print(ln2_out)

# 9) Totals for operations (Block 2)
from numbers import Number

def _sum_scalar_counts(prefix):
    return sum(int(v) for k, v in globals().items()
               if k.startswith(prefix) and isinstance(v, Number))

add_total = int(globals().get("total_add", 0)) + _sum_scalar_counts("add_count")
mul_total = int(globals().get("total_mul", 0)) + _sum_scalar_counts("mul_count")
div_total = _sum_scalar_counts("div_count")
sub_total = _sum_scalar_counts("sub_count")  # should be 0 now

gelu_calls = int(globals().get("count_gelu_in", 0))
exp_calls  = int(globals().get("count_exp", 0))

inv_sqrt_calls = 1 + np.size(y4) + np.size(t4)
